    ...
    <section className="preview-panel" aria-label={t("preview")}>
      <div className="preview-toolbar">
        ...
        <div className="preview-actions">
          <div className="zoom-control"><button type="button" title={t("zoomOut")} aria-label={t("zoomOut")} onClick={() => setZoom((value) => Math.max(40, value - 8))}><Minus size={15} /></button><output aria-live="polite">{zoom}%</output><button type="button" title={t("zoomIn")} aria-label={t("zoomIn")} onClick={() => setZoom((value) => Math.min(100, value + 8))}><Plus size={15} /></button><button type="button" title={t("fitWidth")} aria-label={t("fitWidth")} onClick={fitWidth}><Maximize2 size={15} /></button><button type="button" title={t("view100")} aria-label={t("view100")} onClick={() => setZoom(100)}>100%</button></div>
