      ...
      <div className="item-stack">
        {props.items.map((item, itemIndex) => (
          <article className="experience-card" key={item.id}>
            <header><span className="item-number">{itemIndex + 1}</span><div><strong>{item.position || t("newRole")}</strong><small>{item.company || t("addCompany")}</small></div><div className="item-actions"><button type="button" title={t("duplicateExperience")} onClick={() => props.onDuplicate(item.id)}><Copy size={16} /></button><button type="button" className="danger-icon" title={t("deleteExperience")} onClick={() => props.onRemove(item.id)}><Trash2 size={16} /></button></div></header>
